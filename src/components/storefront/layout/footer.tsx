import Link from 'next/link'
import { Handshake, ClipboardCheck, BadgeCheck, MapPin, Phone, Mail } from 'lucide-react'
import { Logo } from './logo'
import { getWhatsappConfig } from '@/lib/data/whatsapp-config'

/*
 * Footer storefront — structuré en 3 bandes :
 *   1. Réassurance produit (« D'où viennent nos produits ») — 3 points à icône
 *   2. Marque + réseaux · navigation · contact
 *   3. Barre légale
 * Ancres du drawer : #a-propos (bande 1) et #contact (bande 2).
 * Copie = brouillon ; les faits durs (ville, coordonnées, mentions) restent
 * entre crochets.
 */

const TRUST = [
  {
    Icon: Handshake,
    title: 'Fournisseurs sélectionnés',
    description:
      'Des partenaires de confiance, choisis pour leur qualité et suivis dans la durée.',
  },
  {
    Icon: ClipboardCheck,
    title: 'Contrôle qualité',
    description:
      'Chaque article est inspecté à la réception, puis une seconde fois avant l’expédition.',
  },
  {
    Icon: BadgeCheck,
    title: 'Commande fiable',
    description:
      'Commande confirmée par le vendeur, livraison suivie, paiement à la réception.',
  },
]

const NAV = [
  { href: '/catalogue', label: 'Nos boutiques' },
  { href: '/#actualites', label: 'Actualités' },
  { href: '/#a-propos', label: 'À propos' },
  { href: '/compte', label: 'Mon compte' },
  { href: '/panier', label: 'Mon panier' },
]

/**
 * `+226 XXXXXXXX` (E.164) → `+226 XX XX XX XX`. Purement cosmétique : le
 * numéro de vérité reste celui du singleton `whatsapp_config`.
 */
function formatPhone(e164: string): string {
  const m = /^\+226(\d{8})$/.exec(e164.replace(/\s+/g, ''))
  if (!m) return e164
  return `+226 ${m[1].replace(/(\d{2})(?=\d)/g, '$1 ')}`
}

const HEADING =
  'text-[11px] font-semibold uppercase tracking-[0.14em] text-ls-gray-500'

export async function Footer() {
  // Le numéro affiché vient du singleton `whatsapp_config` — même source que le
  // hand-off wa.me. Il était recopié en dur ici : la copie ne dérivait pas
  // encore, mais elle serait devenue fausse au premier changement depuis
  // /admin/whatsapp. Lecture `'use cache'`, donc le layout reste statique.
  const wa = await getWhatsappConfig()
  const contact = [
    { Icon: MapPin, text: 'Wemtenga , vers le Scolastika' },
    {
      Icon: Phone,
      text: wa ? `${formatPhone(wa.numero)} (WhatsApp)` : '+226 60554400',
    },
    { Icon: Mail, text: 'Louisashopping@gmail.com' },
  ]

  return (
    <footer className="ls-dots border-t border-ls-gray-200 bg-ls-white">
      {/* 1 · Réassurance produit */}
      <section
        id="a-propos"
        className="scroll-mt-20 border-b border-ls-gray-200 px-4 py-12 md:px-12"
      >
        <div className="mx-auto max-w-6xl">
          <span
            className="mb-3 block h-[3px] w-8 rounded-full bg-ls-violet"
            aria-hidden
          />
          <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-ls-violet-dark">
            D&apos;où viennent nos produits
          </p>
          <h2 className="mt-2 text-[22px] font-semibold leading-tight text-ls-gray-900">
            Sélectionnés, contrôlés, livrés avec suivi.
          </h2>

          <ul className="mt-7 grid grid-cols-1 gap-6 sm:grid-cols-3">
            {TRUST.map(({ Icon, title, description }) => (
              <li key={title} className="flex items-start gap-3">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-ls-md bg-ls-violet-bg text-ls-violet-dark">
                  <Icon size={20} strokeWidth={1.9} />
                </span>
                <div>
                  <p className="text-[15px] font-semibold text-ls-gray-900">{title}</p>
                  <p className="mt-0.5 text-ls-body text-ls-gray-500">{description}</p>
                </div>
              </li>
            ))}
          </ul>
        </div>
      </section>

      {/* 2 · Marque + réseaux · navigation · contact */}
      <div className="px-4 py-12 md:px-12">
        <div className="mx-auto grid max-w-6xl grid-cols-1 gap-10 md:grid-cols-[1.6fr_1fr_1.2fr]">
          <div>
            <Logo className="h-14 w-auto max-w-[180px]" />
            <p className="mt-3 max-w-xs text-ls-body text-ls-gray-500">
              Boutique en ligne à [Ville — À COMPLÉTER]. Vous commandez, on vous
              livre dans votre zone, et tout se confirme sur WhatsApp.
            </p>
            <div className="mt-4 flex gap-2">
              <SocialLink label="WhatsApp" href="{`https://wa.me/${22660554400}?text=Bonjour%20Louisa%20Shopping,%20j'ai%20une%20question.`}">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.263.489 1.694.625.712.227 1.36.195 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.885 3.488" />
              </SocialLink>
              <SocialLink label="Facebook" href="https://www.facebook.com/profile.php?id=100090359013394&sk=reels_tab">
                <path d="M9.101 23.691v-7.98H6.627v-3.667h2.474v-1.58c0-4.085 1.848-5.978 5.858-5.978.401 0 .955.042 1.468.103a8.68 8.68 0 0 1 1.141.195v3.325a8.623 8.623 0 0 0-.653-.036 26.805 26.805 0 0 0-.733-.009c-.707 0-1.259.096-1.675.309a1.686 1.686 0 0 0-.679.622c-.258.42-.374.995-.374 1.752v1.297h3.919l-.386 2.103-.287 1.564h-3.246v8.245C19.396 23.238 24 18.179 24 12.044c0-6.627-5.373-12-12-12S0 5.417 0 12.044c0 5.628 3.874 10.35 9.101 11.647Z" />
              </SocialLink>
              <SocialLink label="Instagram" href="#">
                <path d="M12 0C8.74 0 8.333.015 7.053.072 5.775.132 4.905.333 4.14.63c-.789.306-1.459.717-2.126 1.384S.935 3.35.63 4.14C.333 4.905.131 5.775.072 7.053.012 8.333 0 8.74 0 12s.015 3.667.072 4.947c.06 1.277.261 2.148.558 2.913.306.788.717 1.459 1.384 2.126.667.666 1.336 1.079 2.126 1.384.766.296 1.636.499 2.913.558C8.333 23.988 8.74 24 12 24s3.667-.015 4.947-.072c1.277-.06 2.148-.262 2.913-.558.788-.306 1.459-.718 2.126-1.384.666-.667 1.079-1.335 1.384-2.126.296-.765.499-1.636.558-2.913.06-1.28.072-1.687.072-4.947s-.015-3.667-.072-4.947c-.06-1.277-.262-2.149-.558-2.913-.306-.789-.718-1.459-1.384-2.126C21.319 1.347 20.651.935 19.86.63c-.765-.297-1.636-.499-2.913-.558C15.667.012 15.26 0 12 0Zm0 2.16c3.203 0 3.585.016 4.85.071 1.17.055 1.805.249 2.227.415.562.217.96.477 1.382.896.419.42.679.819.896 1.381.164.422.36 1.057.413 2.227.057 1.266.07 1.646.07 4.85s-.015 3.585-.074 4.85c-.061 1.17-.256 1.805-.421 2.227a3.81 3.81 0 0 1-.899 1.382 3.744 3.744 0 0 1-1.38.896c-.42.164-1.065.36-2.235.413-1.274.057-1.649.07-4.859.07-3.211 0-3.586-.015-4.859-.074-1.171-.061-1.816-.256-2.236-.421a3.716 3.716 0 0 1-1.379-.899 3.644 3.644 0 0 1-.9-1.38c-.164-.42-.359-1.065-.42-2.235-.045-1.26-.061-1.649-.061-4.844 0-3.196.016-3.586.061-4.861.061-1.17.255-1.814.42-2.234.216-.569.477-.96.9-1.381.419-.419.81-.679 1.379-.895.42-.166 1.051-.361 2.221-.421 1.275-.045 1.65-.06 4.859-.06l.045.03Zm0 3.678a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324ZM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8Zm7.846-10.405a1.44 1.44 0 0 1-2.88 0 1.44 1.44 0 0 1 2.88 0Z" />
              </SocialLink>
            </div>
          </div>

          <nav aria-label="Pied de page">
            <p className={HEADING}>Explorer</p>
            <ul className="mt-3 flex flex-col gap-2 text-ls-body">
              {NAV.map(({ href, label }) => (
                <li key={href}>
                  <Link
                    href={href}
                    className="text-ls-gray-700 underline-offset-2 transition-colors hover:text-ls-violet-dark hover:underline"
                  >
                    {label}
                  </Link>
                </li>
              ))}
            </ul>
          </nav>

          <section id="contact" className="scroll-mt-20">
            <p className={HEADING}>Contact</p>
            <ul className="mt-3 flex flex-col gap-2.5 text-ls-body text-ls-gray-600">
              {contact.map(({ Icon, text }) => (
                <li key={text} className="flex items-start gap-2.5">
                  <Icon
                    size={18}
                    strokeWidth={1.9}
                    className="mt-0.5 shrink-0 text-ls-gray-400"
                  />
                  {text}
                </li>
              ))}
            </ul>
          </section>
        </div>
      </div>

      {/* 3 · Barre légale */}
      <div className="border-t border-ls-gray-200 px-4 pb-[calc(var(--ls-bottom-nav-h)+1.25rem)] pt-5 md:px-12 md:pb-5">
        <div className="mx-auto flex max-w-6xl flex-col gap-2 text-ls-label text-ls-gray-500 sm:flex-row sm:items-center sm:justify-between">
          <p>© 2026 Louisa Shopping</p>
          <div className="flex flex-wrap gap-x-4 gap-y-1">
            <Link href="/mentions-legales" className="hover:text-ls-violet-dark hover:underline">
              Mentions légales
            </Link>
            <Link href="/cgv" className="hover:text-ls-violet-dark hover:underline">
              CGV
            </Link>
            <Link href="/confidentialite" className="hover:text-ls-violet-dark hover:underline">
              Confidentialité
            </Link>
          </div>
        </div>
      </div>
    </footer>
  )
}

function SocialLink({
  label,
  href,
  children,
}: {
  label: string
  href: string
  children: React.ReactNode
}) {
  return (
    <a
      href={href}
      aria-label={label}
      className="flex h-10 w-10 items-center justify-center rounded-full border border-ls-gray-200 text-ls-gray-600 transition-colors hover:border-ls-violet hover:text-ls-violet-dark"
    >
      <svg viewBox="0 0 24 24" fill="currentColor" className="h-[18px] w-[18px]" aria-hidden>
        {children}
      </svg>
    </a>
  )
}
